...
export const pipelineStages: PipelineStage[] = [
  ...
  },
  {
    ...
    description: 'The CPS is doubled by stacking: (1) original CPS records keeping their income, and (2) a copy with PUF-imputed income variables. This creates a rich dataset with both CPS demographics and PUF tax detail. Quantile random forests transfer 67 variables from PUF to CPS.',
    icon: 'link',
    inputSize: '~200,000 persons + ~150,000 PUF returns',
    outputSize: '~400,000 persons (stacked)',
    dataSources: ['CPS 2024', 'PUF 2024 (uprated)'],
    details: [
      '67 variables imputed via quantile random forest (QRF)',
  ...
  },
  {
    ...
    outputSize: '~12,000 households per area',
  ...
  },
  {
    ...
    description: 'Each geographic area\'s dataset is independently recalibrated using L0-regularized optimization to match state-level and congressional district-level administrative targets. The L0 penalty drives most household weights to zero, selecting only the records most representative of each local area.',
    icon: 'map-pin',
    inputSize: '~12,000 households per area',
    outputSize: '50 states + 435 CDs calibrated',
